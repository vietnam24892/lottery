...
const configs = async(req, res) => {
    return res.render("manage/configs.ejs"); 
}
...
const configGet = async(req, res) => {
    ...
    const [configsInfo] = await connection.query('SELECT * FROM config '); 
    return res.status(200).json({
        message: 'Success',
        status: true,    
        datas: configsInfo,
...
const configsUpdate = async(req, res) => {
    let auth = req.cookies.auth;
    let config_home = req.body.config_home;
    let config_wingo = req.body.config_wingo;
    let config_5d = req.body.config_5d;
    let config_k3 = req.body.config_k3;
    if (!auth || !config_home || !config_wingo || !config_5d || !config_k3) {
        return res.status(200).json({
            message: 'Failed',
            status: false,
            timeStamp: timeNow,
    ...
    await connection.query(`UPDATE config SET config_home = ?, config_wingo = ?, config_5d = ?,config_k3 = ?`, [config_home, config_wingo, config_5d,config_k3]);
    return res.status(200).json({
...
const changePassword = async(req, res) => {
    ...
    await connection.query('UPDATE users SET otp = ?, password = ? WHERE phone = ?', [randomNumber(100000, 999999), md5(password), phone]);
    return res.status(200).json({
