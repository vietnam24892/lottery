...
const addK3 = async (game) => {
    try {
        ...
        let [k5DCheck] = await connection.query(`SELECT period FROM k3 WHERE status = 0 AND game = ${game} ORDER BY id DESC LIMIT 1 `);

        if (!k5DCheck[0]) {
            let [k5DCheckGet] = await connection.query(`SELECT period FROM k3 WHERE status = 1 AND game = ${game} ORDER BY id DESC LIMIT 1 `);
