...
const add5D = async(game) => {
    try {
        ...
        let [k5DCheck] = await connection.query(`SELECT period FROM 5d WHERE status = 0 AND game = ${game} ORDER BY id DESC LIMIT 1 `);

        if (!k5DCheck[0]) {
            let [k5DCheckGet] = await connection.query(`SELECT period FROM 5d WHERE status = 1 AND game = ${game} ORDER BY id DESC LIMIT 1 `);
            period = k5DCheckGet[0].period;
...
async function funHanding(game) {
    ...
    for (let i = 0; i < lengthA; i++) {
        ...
        // if (check) {
        //     for (let i = 0; i < result.length; i++) {
        //         const joinNum = sult.includes(result[i]);
        //         if (!joinNum) {                    
                    
        //         }
        //         else
        //         {
        //             countResultA = countResultA + 1;
        //         }
        //     }   
        //     if (lengthA > 0) {
        //         if (countResultA == 0) {
...
const handling5D = async(typeid) => {
    ...
    const [order] = await connection.execute(`SELECT id, phone, bet, price, money, fee,result, amount FROM result_5d WHERE status = 0 AND game = ${game} `);
    for (let i = 0; i < order.length; i++) {
        ...
        let result = String(orders.result).split('');      
        if (check) {
