...
const addWinGo = async (game) => {
    try {    
        ...
        const [winGoNowCheck] = await connection.query(`SELECT period FROM wingo WHERE status = 0 AND game = "${join}" ORDER BY id DESC LIMIT 1 `);

        if (!winGoNowCheck[0]) {
            const [winGoNow] = await connection.query(`SELECT period FROM wingo WHERE status = 1 AND game = "${join}" ORDER BY id DESC LIMIT 1 `);
